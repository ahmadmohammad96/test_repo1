...
def get_gl_entries(filters, accounting_dimensions):
	...
	print('>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>', type(gl_entries))
	# MR.index
	counter = 0
	final_rec = []

	while counter < len(gl_entries)   : 
		if 'Cost of Goods Sold - NF' not in gl_entries[counter]['account'] and 'Creditors - NF' not in gl_entries[counter]['account'] : 
			final_rec.append(gl_entries[counter])
		
		counter +=1  	
	
	gl_entries = final_rec
